...
void testReading (char* file)
{
    ...
    sprintf (dxf_file, "./dxf/%s", file);
    sprintf (svg_file, "./svg/%s.svg", file);

    /* load dxf file into memory */
    std::cout << "Reading file " << file << "...\n";
    Test_CreationClass *creationClass = new Test_CreationClass();
    creationClass->open_svg_file ((const char*)svg_file);
    DL_Dxf* dxf = new DL_Dxf();
    if (!dxf->in (dxf_file, creationClass))  // if file open failed
    {
