...
Test_CreationClass::Test_CreationClass()
{
    ...
    /* add background */
    mxml_node_t *rect;
    rect = mxmlNewElement (g, "rect");
    mxmlElementSetAttr (rect, "x", "0");
    mxmlElementSetAttr (rect, "y", "-1199");
    mxmlElementSetAttr (rect, "width", "1599");
    mxmlElementSetAttr (rect, "height", "1199");
...
void Test_CreationClass::addLayer (const DL_LayerData& data)
{
    ...
    int width;
    get_attributes (&width, &color_r, &color_g, &color_b);
...
void Test_CreationClass::addLine (const DL_LineData& data)
{
    ...
    sprintf (x1, "%6.3f", data.x1 - X0);
    sprintf (y1, "%6.3f", - (data.y1 - Y0));
    sprintf (x2, "%6.3f", data.x2 - X0);
    sprintf (y2, "%6.3f", - (data.y2 - Y0));

    line = mxmlNewElement (g, "line");
...
void Test_CreationClass::addArc (const DL_ArcData& data)
{
    ...
    sprintf (cx, "%6.3f", data.cx - X0);
    sprintf (cy, "%6.3f", -(data.cy - Y0));
    sprintf (r, "%6.3f", data.radius - X0);
    sprintf (ang1, "%6.3f", data.angle1 * 2 * PI / 360);
    sprintf (ang2, "%6.3f", data.angle2 * 2 * PI / 360);

    arc = mxmlNewElement (g, "arc");
...
void Test_CreationClass::addCircle (const DL_CircleData& data)
{
    ...
    circle = mxmlNewElement (g, "circle");
...
void Test_CreationClass::get_attributes (int *width,
...
void Test_CreationClass::addText (const DL_TextData& data)
{
    ...
    sprintf (x, "%6.3f", data.ipx - X0);
    sprintf (y, "%6.3f", - (data.ipy - Y0));

    text = mxmlNewElement (g, "text");
    mxmlElementSetAttr (text, "id", "101");
...
void Test_CreationClass::addEllipse (const DL_EllipseData& data)
{
    ...
    ellipse = mxmlNewElement (g, "ellipse");
