...
#define C104_APDU_SIZE      C104_APDU_LEN + 2 // 255
#define C104_ASDU_SIZE      (C104_APDU_LEN - 4) // 249
#define SC104_DATA_SIZE      (C104_ASDU_SIZE - 6) // 243
...
typedef struct tagT_C104_ASDU
{
    ...
    BYTE	addr_asdu_high;
    BYTE	data[SC104_DATA_SIZE];	/* 信息体报文 */
...
struct IEC104_MASTER
{
    ...
    struct event    ev_timer_WriteDB; //写数据库事件
    ...
    struct timeval  tv_timer_WriteDB; //写数据库定时器
...
#define REAL_VAL_TAG_COUNT     (SC104_DATA_SIZE / REAL_VAL_TAG_SIZE)  // 一个IEC104帧能传17个LongData
...
class WRITE_REAL_VAL_TAG
{
public:
    WRITE_REAL_VAL_TAG ();
    ~WRITE_REAL_VAL_TAG ();
    void push (REAL_VAL_TAG tag);
    void pop();

private:
    std::vector<REAL_VAL_TAG> queue; // db写队列
    int pos; // 记录写队列位置
    pthread_mutex_t mutex; //写队列mutex
};
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
