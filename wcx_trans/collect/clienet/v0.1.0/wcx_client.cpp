...
struct event                ev_timer_connect; //timer 时间对象
...
static char     tag_file_optimized[100]; //优化点表文件
...
static void iec104_master_connect()
{
    ...
    for (i = 0; i < IEC104_SLAVE_IP_NUMS; i++)
    {
        if (iec104[i]->is_connect == FALSE)
        {
            ...
            {
                printf ("+master: connect to %s successful!\n", iec104[i]->ip_addr);
                //--->Init..
                iec104[i]->i = i;
                iec104[i]->SendNum = 0;
                iec104[i]->RecvNum = 0;
...
static void iec104_send_msg (int i)
{
    ...
    if (iec104[i]->SendLen > 0)
    {
        ...
        {
            //iec104[i]->SendBuf[2] = (iec104[i]->SendNum % 256) << 1;
            iec104[i]->SendBuf[2] = (iec104[i]->SendNum % 256);
            iec104[i]->SendBuf[3] = iec104[i]->SendNum / 256;
            //iec104[i]->SendBuf[4] = (iec104[i]->RecvNum % 256) << 1;
        ...
        */
        ...
        /*if (debug == TRUE)*/
        /*{*/
        /*IEC104_Display (IEC104->SendBuf, IEC104->usSendLen, 1);*/
        /*}*/
...
static void on_read (int fd, short ev, void *arg)
{
    ...
    else //I_format
    {
        ...
        if (1)
        {
            ...
            {
                REAL_VAL_TAG *ptag;
                ptag = (REAL_VAL_TAG*) (&p_asdu->data[0]);
                for (i = 0; i < INFO_NUMS; i++)
                {
                    ...
                    double dvalue = value;
                    dvalue = dvalue / 1000;
                    printf ("!#tag_id:% 5d, value: %.5f, year:%4d, month:%d, week_day:%d, hour:%d, min:%d, sec:%d\n",
                            info_addr,
                            dvalue,
                            ptag->year + 1900,
                            ptag->month,
