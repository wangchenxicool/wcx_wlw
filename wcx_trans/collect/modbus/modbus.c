...
void error_treat (modbus_param_t *mb_param, int code, const char *string)
{
    ...
    /*log_info ("error_treat:%s (%0X)", string, -code);*/
...
static void modbus_flush (modbus_param_t *mb_param)
{
...
unsigned int compute_response_length (modbus_param_t *mb_param, uint8_t *query , uint8_t data_type)
{
    int length;
    int offset;
...
static int build_query_basis_rtu (int slave, int function, int start_addr, int nb, uint8_t *query)
{
    query[0] = slave;
    query[1] = function;
...
int build_query_basis (modbus_param_t *mb_param,
...
int check_crc16 (modbus_param_t *mb_param, uint8_t *msg, const int msg_length)
{
    int ret;
    uint16_t crc_calc;
...
int modbus_send (modbus_param_t *mb_param, uint8_t *query, int query_length)
{
    int i;
    int ret;
    uint16_t s_crc;
    ...
    if (mb_param->debug)
    {
        /*printf ( "\033[31;40;5m send:\033[0m" );*/
    ...
    {
        ret = SOCKET_FAILURE;
        error_treat (mb_param, ret, "modbus_send: Write socket failure");
    }
...
/* Waits a reply from a modbus slave or a query from a modbus master.
   ...
   a negative value.
*/
int receive_msg (modbus_param_t *mb_param, int msg_length_computed, uint8_t *msg,
                 int select_time)
{
    int select_ret;
    int read_ret;
    ...
    while (select_ret)
    {
        ...
        if (read_ret == 0)
        {
            write_to_log ("/var/log", "wcx_modbus.log", "receive_msg: read_ret is 0, CONNECTION_CLOSED![625]", true);
        ...
        else
        {
            ...
            case BYTE:
                ...
                {
                    error_treat (mb_param, INVALID_DATA, "receive: Too many data");
                    return INVALID_DATA;
                }
                state = COMPLETE;
                break;
...
int rcv_msg (modbus_param_t *mb_param, uint8_t *msg, int select_time, int wait_time)
{
    int select_ret;
    int read_ret;
    fd_set rfds;
    ...
    if (mb_param->type_com == RTU)
    {
        /* Returns msg_length on success and a negative value on
        failure */
        return check_crc16 (mb_param, msg, read_ret);
    }
    else
    {
        /* OK */
                          ...
                          int select_time)
{
    ...
    return receive_msg (mb_param, MSG_LENGTH_UNDEFINED, query, select_time);
}
...
/* Receives the response and checks values (and checksum in RTU).
   ...
   these values. */
int modbus_receive (modbus_param_t *mb_param, uint8_t *query, uint8_t *response,
...
again:
    response_length_computed = compute_response_length (mb_param, query, data_type);
                           ...
                           int start_addr, int nb, uint8_t *data_dest, int select_time)
{
    ...
    if (ret > 0)
    {
        ...
        ret = modbus_receive (mb_param, query, response, UINT16, select_time);
        if (ret < 0)
        {
            return ret;
        }
                      ...
                      int nb, uint8_t *data_dest, int select_time)
{
    ...
    status = read_io_status (mb_param, FC_READ_COIL_STATUS, start_addr, nb, data_dest, select_time);
    if (status > 0)
    {
        status = nb;
    }
                           ...
                           int nb, uint32_t *data_dest, uint8_t data_type, int select_time)
{
    ...
    if (ret > 0)
    {
        ...
        ret = modbus_receive (mb_param, query, response, data_type, select_time);
                            ...
                            uint32_t *data_dest, uint8_t data_type, int select_time)
{
    ...
    status = read_registers (mb_param, FC_READ_HOLDING_REGISTERS, start_addr, nb, data_dest, data_type, select_time);
    return status;
}
...
static int set_single (modbus_param_t *mb_param, int function, int addr, int value,
                       int select_time)
{
    int ret;
    int query_length;
    ...
    query_length = build_query_basis (mb_param, function, addr, value, query);
...
int force_single_coil (modbus_param_t *mb_param, int coil_addr, int state, int select_time)
{
    int status;
    ...
    status = set_single (mb_param, FC_FORCE_SINGLE_COIL, coil_addr, state, select_time);

    return status;
}

/* Sets a value in one holding register in the slave device */
int preset_single_register (modbus_param_t *mb_param, int reg_addr, int value, int select_time)
{
    int status;

    status = set_single (mb_param, FC_PRESET_SINGLE_REGISTER, reg_addr, value, select_time);
                               ...
                               const uint16_t *data_src, int select_time)
{
    ...
    query_length = build_query_basis (mb_param, FC_PRESET_MULTIPLE_REGISTERS, start_addr, nb, query);
    byte_count = nb * 2;
    query[query_length++] = byte_count;
...
int report_slave_id (modbus_param_t *mb_param, uint8_t *data_dest, int select_time)
{
    int ret;
    int query_length;
    uint8_t query[MIN_QUERY_LENGTH];
